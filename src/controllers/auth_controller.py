...
def register_user(user_type):
    try:
        ...
        if user_type not in ["patient", "doctor"]:
            return jsonify({"message": f"User type '{user_type}' not valid. URL must include '/auth/register/patient' or '/auth/register/doctor'."})#, 400?
        ...
        if err.orig.pgcode == errorcodes.NOT_NULL_VIOLATION:
            ...
            return jsonify({"error": f"The column {err.orig.diag.column_name} is required."}), 400

        if err.orig.pgcode == errorcodes.UNIQUE_VIOLATION:
            return jsonify({"error": "Email address must be unique."}), 400
...
def login_user(user_type):
    ...
    if user_type not in ["patient", "doctor"]:
        return jsonify({"message": f"User type '{user_type}' not valid. URL must include '/auth/login/patient' or '/auth/login/doctor'."})#, 400?
    ...
    if user_type == "patient":
        ...
        if patient and bcrypt.check_password_hash(patient.password, body_data.get("password")):
            ...
            return jsonify({"email": patient.email, "is_admin": patient.is_admin, "token": token})
        ...
        if doctor and bcrypt.check_password_hash(doctor.password, body_data.get("password")):
            ...
            return jsonify({"email": doctor.email, "is_admin": doctor.is_admin, "token": token})
